...
");
        ...
        IPropertySymbol p)
    {
        ...
        bool isEnumerable = type.Name != "String" && (isList || isArray || p.IsICollection());
        ...
        string displayItemType = isEnumerable 
                                    ? p.Type.ToDisplayString()  // TODO: get the generics type
                                    : displayType;
        ...
        else
        {
            ...
            builder.AppendLine($"{indent}private static {displayItemType} Cast_{name}(TryGetValue tryGet)");
            builder.AppendLine($"{indent}{{");
            builder.AppendLine($"{indent1}// if (m is IDictionary<string, object?> d)");
            builder.AppendLine($"{indent1}// {{");
            builder.AppendLine($"{indent2}// var val = {displayItemType}.FromDictionary(d);");
